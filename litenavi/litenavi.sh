...
LN_Screen_Rendering (){ #отрисовка списка
    LN_Info_Area=$(($(printf "%s\n" ${Module_Info_1} | wc -l)+$(printf "%s\n" ${Module_Info_2} | wc -l)+1)) #запоминаем, сколько строк дополнительной информации
...
LN_Help="<j>-down <k>-up <SPACE>-select <ENTER>-apply selected"
: ${Module_Name:=files.lnm} #модуль по умолчанию - работа с файлами
: ${Module_Info_1:=echo "${LN_Help}"} #первый информационный блок по умолчанию - пустая строка
: ${Module_Info_2:=echo "${Module_Help}"} #второй информационный блок по умолчанию - пустая строка
