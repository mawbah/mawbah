...
LN_Screen_Rendering (){ #отрисовка списка
    LN_Info_Area=$(($((echo ${Module_Caption}; eval ${Module_Info_1}; eval ${Module_Info_2}) | wc -l)+3)) #запоминаем, сколько строк дополнительной информации
    ...
    echo ${Module_Caption} #печатаем заголовок
    echo
...
: ${Module_Caption:=" "} #первый информационный блок по умолчанию - пустая строка
